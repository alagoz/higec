...
class hier_binary_tree:
    ...
    def __init__(self,
        ...
        self.hc_type = 'lcpn' # 'lcpn', 'lcpn+', 'lcpn+f', 'lcl+', or 'lcn+'
    ...
    def assign_all_nodes_using_pnodes(self,verbose=False):
        ...
        if verbose: self.display_extended_linkage(hc_type='all')
        return all_nodes
    ...
    def assign_lcn_nodes(self,verbose=False):
        ...
        if verbose: self.display_extended_linkage(hc_type='lcn')
        return lcns
    ...
    def assign_levels(self, verbose=False):
        ...
        if verbose: self.display_extended_linkage(hc_type='lcl')
        return levels
    ...
    def find_path_for_leaves(self,hc_type='lcpn'):
        # find pathway for each flat label
        path_branch = []
        path_parent = []
        if 'lcpn' in hc_type or 'gc' in hc_type:
            PNs=self.assert_parent_nodes()
    ...
    def assign_weights_for_branches(self,y_train=None,hc_type='lcpn+',scale_='lin',verbose=False):
        ...
        for u_ in units:
            if 'lcn' in hc_type and len(u_.branch_ids)==1:
    ...
    def display_extended_linkage(self,hc_type='lcn'):
        kw_={'lcn':'LCN','lcn+':'LCN','lcn+f':'LCN','lcpn':'LCPN','lcpn+':'LCPN','lcpn+f':'LCPN','all':'all hierarchical'}
        
        units = self.assign_units(hc_type=hc_type)
        if hc_type in ['lcn','lcn+','lcn+f','lcpn','lcpn+','lcpn+f','all']:
            print(f'Extended linkage table for {kw_[hc_type]} nodes:')
            for u_ in units:
    ...
    def compose_global_training_set(self,x_train,y_train,hc_type='gc',verbose=False):
        units = self.assign_units(hc_type=hc_type)        
    ...
    def fit(self, clf_base, x_train, y_train=None, hc_type='lcpn+', rseed=None, multi_process=False):
        if y_train is None: y_train=self.y_train
        ...
        if hc_type in ['lcpn','lcpn+','lcpn+f','lcl+','lcl+f','lcn+','lcn+f']:
            """
            ...
            if hc_type in ['lcpn+f','lcn+f','lcl+f']:
                """
            ...
            """
            x_tr_glob,y_tr_glob=self.compose_global_training_set(x_train,y_train,hc_type='lcpn',verbose=0)
    ...
    def predict(self, x_test, hc_type='lcpn+', weights='none', proportion='direct', chain_type='prod', usefulness=False, multi_process=False):
        ...
        elif hc_type in ['lcpn+','lcn+','lcl+']:
            """
            Label each sample not with "top-down" but with taking into account all classifiers
            in the parent nodes. Error propagation is thereby expected to be minimized. 
            """
        ...
        elif hc_type in ['lcpn+f','lcn+f','lcl+f','gc+f']:
            pred_proba_all=np.zeros((x_test.shape[0],self.n_branches))
            if hc_type=='lcpn+f':
                for u_ in units:
